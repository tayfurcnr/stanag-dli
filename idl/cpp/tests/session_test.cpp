...
/**
 * @brief Mock Transport to bridge between DliSession and MockNetwork
 */
class MockTransport : public ITransport {
public:
    std::function<bool(const uint8_t*, size_t)> send_func;
    
    bool start() override { return true; }
    void stop() override {}
    void flush() override {}
    void poll(DataCallback) override {}
...
int main() {
    ...
    // VSM is the sender
    dli::DliSession vsm(vsm_dispatcher);
    MockTransport vsm_tx;
    vsm_tx.send_func = [&](const uint8_t* b, size_t s) {
        std::cout << "  [DEBUG] vsm -> cucs (" << s << " bytes)" << std::endl;
        return net.send(b, s);
    };
    vsm.add_transport(&vsm_tx);
    
    // CUCS is the receiver
    dli::DliSession cucs_final(cucs_dispatcher);
    MockTransport cucs_tx;
    cucs_tx.send_func = [&](const uint8_t* b, size_t s) {
        std::cout << "  [DEBUG] cucs -> vsm (" << s << " bytes)" << std::endl;
        vsm.process_incoming(b, s); 
        return true; 
    };
    cucs_final.add_transport(&cucs_tx);
